...
function Show-Status {
    ...
    # Verificar WP-CLI estandarizado
    $wpDevLocation = Invoke-SSHCommand -Server $DevServer -Command "which wp"
    $wpStageLocation = Invoke-SSHCommand -Server $StageServer -Command "which wp"
    
    Write-ColorLog "WP-CLI Locations:" "Cyan"
    Write-ColorLog "  Dev: $wpDevLocation" $(if($wpDevLocation -eq "/usr/local/bin/wp") { "Green" } else { "Yellow" })
    Write-ColorLog "  Stage: $wpStageLocation" $(if($wpStageLocation -eq "/usr/local/bin/wp") { "Green" } else { "Yellow" })
...
function Test-WordPress {
    ...
    # MEJORADO: Con debugging completo
    $testCommand = "cd $Path && wp core is-installed"
    Write-ColorLog "  Ejecutando: $testCommand" "Gray"
    
    $wpTest = Invoke-SSHCommand -Server $Server -Command $testCommand
    
    # Debugging detallado
    Write-ColorLog "  Resultado crudo: '$wpTest'" "Gray"
    Write-ColorLog "  Es null: $($wpTest -eq $null)" "Gray"
    Write-ColorLog "  LASTEXITCODE: $LASTEXITCODE" "Gray"
    
    # Test alternativo si falla el principal
    if ($wpTest -eq $null) {
        Write-ColorLog "  Intentando test alternativo..." "Yellow"
        $altTest = Invoke-SSHCommand -Server $Server -Command "cd $Path && wp --info"
        Write-ColorLog "  wp --info resultado: '$altTest'" "Gray"
        
        if ($altTest -ne $null) {
            Write-ColorLog "  WP-CLI funciona, problema con is-installed" "Yellow"
            # Intentar test de version como alternativa
            $versionTest = Invoke-SSHCommand -Server $Server -Command "cd $Path && wp core version"
            if ($versionTest -ne $null) {
                Write-ColorLog "$EnvName WordPress: OK (version test passed)" "Green"
                return $true
            }
        }
    }
...
function Execute-Rollback {
    ...
    # Rollback BD - ESTANDARIZADO: wp funciona igual en ambos
...
Write-ColorLog "WordPress Deployment Manager - ESTANDARIZADO" "Cyan"
Write-ColorLog "============================================" "White"
...
Write-ColorLog "WP-CLI: Estandarizado en /usr/local/bin/wp" "Green"
Write-ColorLog "============================================" "White"
