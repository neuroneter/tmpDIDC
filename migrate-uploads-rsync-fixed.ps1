...
function Execute-UploadsRsyncMigration {
    ...
    if ($DryRun) {
        ...
        # Comando RSYNC remoto con dry-run
        $dryRunCmd = "sshpass -p '$Password' rsync -avz --dry-run --itemize-changes -e 'ssh -o StrictHostKeyChecking=no' $Username@$DevServer`:$DevPath/ $StagePath/"
    ...
    $changesCmd = "sshpass -p '$Password' rsync -avz --dry-run --itemize-changes -e 'ssh -o StrictHostKeyChecking=no' $Username@$DevServer`:$DevPath/ $StagePath/"
    ...
    # Paso 3: Migración real con RSYNC REMOTO
    Write-ColorLog "Paso 3/4: === EJECUTANDO RSYNC REMOTO MIGRATION ===" "Red"
    Write-ColorLog "Comando: sshpass rsync -avz --delete Dev->Stage uploads" "Gray"
    
    $rsyncCmd = "sshpass -p '$Password' rsync -avz --delete --progress -e 'ssh -o StrictHostKeyChecking=no' $Username@$DevServer`:$DevPath/ $StagePath/"
    $rsyncResult = Invoke-SSHCommand -Server $StageServer -Command $rsyncCmd
    
    if (-not $rsyncResult -and $LASTEXITCODE -ne 0) {
        Write-ColorLog "ERROR CRÍTICO: Fallo en RSYNC REMOTO migration" "Red" "ERROR"
    ...
    if ($devFilesAfter -eq $stageFilesAfter) {
        Write-ColorLog "SUCCESS: Migración RSYNC REMOTO completada - Conteos coinciden perfectamente" "Green"
